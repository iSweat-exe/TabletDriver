...
impl ReportParser for TenMoonParser {
    fn parse(&self, data: &[u8]) -> Option<TabletData> {
        ...
        if data[11] != 0xFF {
            ...
            if data[12] == 0x31 { buttons |= 1 << 0; }
            if data[12] == 0x33 && (data[11] & 0x80) == 0 { buttons |= 1 << 1; }
            if data[12] == 0x33 && (data[11] & 0x40) == 0 { buttons |= 1 << 2; }
            if data[12] == 0x33 && (data[11] & 0x20) == 0 { buttons |= 1 << 3; }
            if data[12] == 0x33 && (data[11] & 0x10) == 0 { buttons |= 1 << 4; }
            if data[12] == 0x33 && (data[11] & 0x08) == 0 { buttons |= 1 << 5; }
            if data[12] == 0x23 { buttons |= 1 << 6; }
            if data[12] == 0x32 { buttons |= 1 << 7; }
            ...
            let pressure = if pre_pressure >= pressure_offset {
                ...
                if 0x0672 > adjusted { 0x0672 - adjusted } else { 0 }
            } else {
                0x0672
            };
