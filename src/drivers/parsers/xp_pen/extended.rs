...
fn parse_gen2(data: &[u8], raw: String) -> Option<TabletData> {
    ...
    let pressure = (u16::from_le_bytes([data[6], data[7]]) & 0xBFFF) | (((data[13] & 0x01) as u16) << 13);
...
impl ReportParser for XpPenGen2Parser {
    fn parse(&self, data: &[u8]) -> Option<TabletData> {
        ...
        if data[1] == 0xF0 { return parse_aux(data, raw, 2); }
        if (data[1] & 0xF0) == 0xA0 { return parse_gen2(data, raw); }
        standard_parse(data)
    }
}
...
impl ReportParser for XpPenDeco03Parser {
    fn parse(&self, data: &[u8]) -> Option<TabletData> {
        ...
        let raw = data.iter().map(|b| format!("{:02X}", b)).collect::<Vec<_>>().join(" ");
        if data[1] == 0xC0 { return None; }
        if data[1] == 0xF0 { return parse_aux(data, raw, 2); } // Wheel parsed as aux
        if (data[1] & 0x10) != 0 { return parse_aux(data, raw, 2); }
        standard_parse(data)
    }
}
...
impl ReportParser for XpPenOffsetPressureParser {
    fn parse(&self, data: &[u8]) -> Option<TabletData> {
        ...
        if data[1] == 0xC0 { return None; }
        if (data[1] & 0x10) != 0 { return parse_aux(data, raw, 2); }
        if data.len() >= 10 { parse_offset_pressure(data, raw, true) } else { parse_offset_pressure(data, raw, false) }
    }
}
...
impl ReportParser for XpPenOffsetAuxParser {
    fn parse(&self, data: &[u8]) -> Option<TabletData> {
        ...
        let raw = data.iter().map(|b| format!("{:02X}", b)).collect::<Vec<_>>().join(" ");
        if data[1] == 0xC0 { return None; }
        if (data[1] & 0x20) != 0 { return parse_aux(data, raw, 4); }
        standard_parse(data)
    }
}
...
mod tests {
    ...
    fn test_xp_pen_gen2() {
        ...
        let data: [u8; 14] = [0, 0xA2, 0x02, 0, 0x04, 0, 0x01, 0x00, 10, 20, 0x01, 0x03, 0, 0];
