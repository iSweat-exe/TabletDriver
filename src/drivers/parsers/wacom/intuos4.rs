...
impl Intuos4Parser {
    ...
    fn parse_mouse(&self, data: &[u8], raw: String) -> Option<TabletData> {
        ...
        if (data[6] & 0x01) != 0 { buttons |= 1 << 0; }
        if (data[6] & 0x04) != 0 { buttons |= 1 << 1; }
        if (data[6] & 0x02) != 0 { buttons |= 1 << 2; }
        if (data[6] & 0x08) != 0 { buttons |= 1 << 3; }
        if (data[6] & 0x10) != 0 { buttons |= 1 << 4; }
