...
impl IntuosV3Parser {
    ...
    fn parse_tablet(&self, data: &[u8], raw: String) -> Option<TabletData> {
        ...
        let tilt_x = if (data[9] & 0x80) != 0 { (data[9] as i16 - 0xFF) as i8 } else { data[9] as i8 };
        let tilt_y = if (data[11] & 0x80) != 0 { (data[11] as i16 - 0xFF) as i8 } else { data[11] as i8 };
        ...
        Some(TabletData {
            ...
            x, y, pressure, tilt_x, tilt_y, buttons, eraser, hover_distance,
            raw_data: raw, is_connected: true, ..Default::default()
        })
    }
    ...
    fn parse_extended(&self, data: &[u8], raw: String) -> Option<TabletData> {
        ...
        Some(TabletData {
            ...
            pressure, tilt_x, tilt_y, buttons, eraser, hover_distance,
            raw_data: raw, is_connected: true, ..Default::default()
        })
    }
    ...
    fn parse_aux(&self, data: &[u8], raw: String) -> Option<TabletData> {
        ...
        let b1 = data[1];
        let b2 = data[3];
        if (b1 & 1) != 0 { buttons |= 1 << 0; }
        if (b1 & 2) != 0 { buttons |= 1 << 1; }
        if (b1 & 4) != 0 { buttons |= 1 << 2; }
        if (b1 & 8) != 0 { buttons |= 1 << 3; }
        if (b2 & 1) != 0 { buttons |= 1 << 4; }
        if (b1 & 16) != 0 { buttons |= 1 << 5; }
        if (b1 & 32) != 0 { buttons |= 1 << 6; }
        if (b1 & 64) != 0 { buttons |= 1 << 7; }
...
mod tests {
    ...
    fn test_intuos_v3_tablet_report() {
        ...
        data[3] = 0x01; data[4] = 0x00; // X = 1
        data[7] = 0xAA; data[8] = 0x00; // Pressure = 170
