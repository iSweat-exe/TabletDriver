...
impl Intuos3Parser {
    ...
    fn parse_mouse(&self, data: &[u8], raw: String) -> Option<TabletData> {
        ...
        if (data[8] & 0x04) != 0 { buttons |= 1 << 0; }
        if (data[8] & 0x10) != 0 { buttons |= 1 << 1; }
        if (data[8] & 0x08) != 0 { buttons |= 1 << 2; }
        if (data[8] & 0x20) != 0 { buttons |= 1 << 3; }
        if (data[8] & 0x40) != 0 { buttons |= 1 << 4; }

        Some(TabletData {
            status: "Mouse".to_string(),
            x, y, buttons, raw_data: raw, is_connected: true, ..Default::default()
        })
    }
    ...
    pub(crate) fn parse_aux(&self, data: &[u8], raw: String, extra: bool) -> Option<TabletData> {
        ...
        if (b5 & 1) != 0 { buttons |= 1 << 0; }
        if (b5 & 2) != 0 { buttons |= 1 << 1; }
        if (b5 & 4) != 0 { buttons |= 1 << 2; }
        if (b5 & 8) != 0 { buttons |= 1 << 3; }
        if (b6 & 1) != 0 { buttons |= 1 << 4; }
        if (b6 & 2) != 0 { buttons |= 1 << 5; }
        if (b6 & 4) != 0 { buttons |= 1 << 6; }
        if (b6 & 8) != 0 { buttons |= 1 << 7; }
        
        if extra {
            if (b5 & 16) != 0 { buttons |= 1 << 8; }
            if (b6 & 16) != 0 { buttons |= 1 << 9; }
        }
